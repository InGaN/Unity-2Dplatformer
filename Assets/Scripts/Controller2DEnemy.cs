...
public class Controller2DEnemy : RaycastController {
    ...
    {
        float directionX = collisions.faceDirection; //Mathf.Sign(velocity.x);
        ...
        for (int i = 0; i < verticalRayCount; i++)
        {
            ...
            if (hit) {
                if(hit.collider.tag == "Through") {
                    if(directionY == 1 || hit.distance == 0) { // (hit.distance == 0) prevents player sliding just through
                        continue;
                    }
                    ...
                    if(playerInput.y == -1) {
                        ...
                        Invoke("ResetFallingThroughPlatform", 0.5f); // call after half a second
                        continue;
                    }
                }
    ...
    private void ClimbSlope(ref Vector3 velocity, float slopeAngle) 
    {
        ...
        if (velocity.y > climbVelocityY) { // jumping from slope
            print("Jump from slope");
        }
        else {
    ...
    private void DescendSlope(ref Vector3 velocity)
    {
        ...
        if(hit) {
            float slopeAngle = Vector2.Angle(hit.normal, Vector2.up);
            if (slopeAngle != 0 && slopeAngle <= maxDescendAngle) { //flat surface
